...
class SignalProcessor:
    ...
    async def _process_new_signals(self, messages, profile):
        ...
        for msg_id, msg_content, msg_timestamp in messages:
            ...
            if self.config.sentiment_filter['enabled']:
                ...
                threshold = self.config.sentiment_filter['sentiment_threshold'] if is_call else \
                self.config.sentiment_filter['put_sentiment_threshold']
    ...
    async def _execute_trade_from_signal(self, signal, profile):
        ...
        try:
            ...
            if quantity == 0:
                logging.warning(
                    f"Trade for {contract.localSymbol} vetoed. Not enough funds to purchase a single contract at ${ask_price}.")
                return
    ...
    def _on_order_filled(self, trade):
        ...
        position_details = {
            ...
            'profile_name': self.get_profile_for_position(contract)
        }
    ...
    async def _evaluate_dynamic_exit(self, conId):
        ...
        if profile is None or data is None or data.empty: return
        ...
        for exit_type in profile['exit_strategy']['exit_priority']:
            ...
            if exit_type == "atr_trail" and profile['exit_strategy']['trail_method'] == 'atr':
                ...
                if is_call and last_rsi > rsi_settings['overbought_level']:
                    exit_reason = f"RSI crossed overbought ({last_rsi:.2f} > {rsi_settings['overbought_level']})"
                elif not is_call and last_rsi < rsi_settings['oversold_level']:
                    exit_reason = f"RSI crossed oversold ({last_rsi:.2f} < {rsi_settings['oversold_level']})"
    ...
    def get_profile_for_position(self, contract):
        # Placeholder: Assumes first enabled profile. Needs robust logic for multi-profile use.
        for profile in self.config.profiles:
            if profile['enabled']:
