            ...
            try:
                ...
                try:
                    ...
                    if age_seconds > self.config.signal_max_age_seconds:
                        logging.debug(
                            f"Ignoring old message {msg_id} (age: {age_seconds:.1f}s > {self.config.signal_max_age_seconds}s)")
                        continue
                ...
                if not parsed_signal:
                    ...
                    logging.info(
                        f"   Profile settings: assume_buy={profile.get('assume_buy_on_ambiguous')}, ambiguous_expiry={profile.get('ambiguous_expiry_enabled')}")
                    continue

                logging.info(
                    f"✅ PARSED: {parsed_signal['ticker']} {parsed_signal['strike']}{parsed_signal['contract_type']} {parsed_signal['expiry_date']}")
                ...
                # Execute the trade
                logging.info(
                    f"✅ SIGNAL ACCEPTED - {parsed_signal['ticker']} {parsed_signal['strike']}{parsed_signal['contract_type']} {parsed_signal['expiry_date']}")
    ...
    async def _close_position(self, conId, exit_reason, exit_price):
        ...
        try:
            ...
            if order:
                ...
                logging.info(
                    f"✅ CLOSED: {quantity} {contract.localSymbol} @ ${exit_price:.2f} | P&L: ${pnl:.2f} ({pnl_percent:.1f}%)")
                ...
                if ghost_positions:
                    ...
                    for ghost_conId in ghost_positions:
                        ...
                        try:
                            ...
                            logging.warning(
                                f"🔨 FORCE-CLOSING GHOST: {action} {quantity} of {contract.localSymbol} (conId: {ghost_conId})")
